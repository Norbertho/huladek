...
(function () {
  ...
  jigsaw.Jigsaw = Class.extend({
    init: function (opts) {
      ...
      eventBus.on(jigsaw.Events.JIGSAW_SHUFFLEBTN, this.shuffle.bind(this));
    ...
    draw_image: function (img, w, h) {
      ...
      if (max_w > this.width() || max_h > this.height() - TOOLBAR_HEIGHT) {
        var ratio = Math.min(
          this.width() / max_w,
          (this.height() - TOOLBAR_HEIGHT) / max_h
        );
        max_w *= ratio;
        max_h *= ratio;
      }
...
(function () {
  ...
  jigsaw.UI = Class.extend({
    init: function (eventBus, parts) {
      ...
      eventBus.on(jigsaw.Events.JIGSAW_SHUFFLEBTN, this.init_clock.bind(this));
  ...
  function init_events(self, eventBus) {
    ...
    Cevent.addEventListener("openMenu", "mousedown", function (e) {
        var target = e.target || e.srcElement;
        if (jigsaw.Events[target.id]) {
          e.preventDefault();
          eventBus.emit(jigsaw.Events[target.id]);
...
(function () {
  jigsaw.Events = {
    ...
    JIGSAW_SHUFFLEBTN: "JigsawShuffleEvent",
